...
public class tk2dUIScrollableArea : MonoBehaviour
{
    ...
    public float ContentLength
    {
        get 
		{
			//很多地方要用contentLength - visibleAreaLength来当除数，所以这里加个特殊处理
			//chenlin 修改
			if(contentLength == visibleAreaLength)
			{
				return contentLength + 0.01f;
			}
        ...
        set 
        {
			...
			if(value <= visibleAreaLength)
			{
				targetLength = visibleAreaLength + 0.01f;
			}
			ContentLengthVisibleAreaLengthChange(contentLength, targetLength,visibleAreaLength,visibleAreaLength);
        }
    }
	...
	/// 滑动到边界，是否还能滑动 
	/// </summary>
	public bool allowTouchScrolling = true;

	/// <summary>
	/// 滑动到边界，是否还能滑动 
	/// </summary>
	public bool allowScrollThreshold = true;
    ...
    public bool isBackgroundButtonDown = false;
	...
	public bool isSwipeScrollingInProgress = false;
	...
	public float swipeCurrVelocity = 0; //velocity of current frame (used for inertia swipe scrolling)
    private float snapBackVelocity = 0;
	
	protected float ContentDestOffset
	{
		get
		{
			if (scrollAxes == Axes.XAxis)
			{
				return swipeScrollingContentDestLocalPos.x;
			}
			else 
			{
				return swipeScrollingContentDestLocalPos.y;
			}
		}
		set 
		{
			if (scrollAxes == Axes.XAxis)
			{
				swipeScrollingContentDestLocalPos.x = value;
			}
			else 
			{
				swipeScrollingContentDestLocalPos.y = value;
			}
		}
	}

	protected void RemoveBackgroundOverUpdate()
	{
    ...
    public float Value
    {
        ...
        set
        {
            ...
            if (value != percent)
            {
				...
				//chenlin 修改，这个先保证监听者得到正确的offset
				SetContentPosition();
    ...
    public Vector3 ContentContainerOffset {
        ...
        set 
		{
			if(false == allowScrollThreshold)
			{
				//add by chenlin
				Vector3 tmpValue = value;
				float search = scrollAxes == Axes.XAxis ? value.x : value.y ; 
				if(search <= 0.0f)
				{
					if(scrollAxes == Axes.XAxis)
					{
				...
				if(search >= ContentLength - VisibleAreaLength)
				{
					if(scrollAxes == Axes.XAxis)
					{
						tmpValue.x = ContentLength - VisibleAreaLength;
					}
					else
					{
						tmpValue.y = ContentLength - VisibleAreaLength;
					}
				}
				contentContainer.transform.localPosition = Vector3.Scale(new Vector3(-1, 1, 1), tmpValue);
			}
			else
			{
